...
with lib; let
  ...
  normalizeTtyConfig = config:
    if isBool config
    then
      if config
      then generateDefaultTtys 6
      else []
    else if isList config
    then config
    else throw "Invalid TTY configuration type";
  ...
  in
    nameValuePair "getty@tty${ttyNum}" {
      ...
      serviceConfig = {
        ExecStart = mkForce (
          if parsed.autologin && user != null
          then "${getExe' pkgs.util-linux "agetty"} --login-program ${pkgs.shadow}/bin/login --autologin ${user} --noclear %I $TERM"
          else "${getExe' pkgs.util-linux "agetty"} --login-program ${pkgs.shadow}/bin/login --noclear %I $TERM"
        );
        Type = "idle";
        Restart = "always";
        RestartSec = "0";
  ...
  buildKmsconArgs = kmsconConfig: let
  ...
  in
    nameValuePair "kmsconvt@tty${ttyNum}" {
      ...
      serviceConfig = {
        ExecStart = mkForce (
          if parsed.autologin && user != null
          then "${getExe pkgs.kmscon} ${argsStr} -- ${pkgs.shadow}/bin/login -f ${user}"
          else "${getExe pkgs.kmscon} ${argsStr} -- ${pkgs.shadow}/bin/login"
        );
        Type = "simple";
        Restart = "always";
        RestartSec = "0";
  ...
  validateUserExists = spec: let
    user = extractUser spec;
  in
    if user != null
    then user != "" || throw "Empty username in: ${spec}"
    else true;
in {
